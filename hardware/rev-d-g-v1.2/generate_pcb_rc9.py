...
BOARD_FILE=ROOT/"RevD_G_v1_2_PCB_RC9.kicad_pcb"
...
# RC9 board outline 120 x 85 mm. Size is intentionally relaxed until routing is proven.
...
# Buck local loop: SW -> L801.  VOS returns around the package edge so it does
# not cross the inductor SW pad.
locked_direct("BUCK_SW",("U7","7"),("L801","1"),0.30)
pout=pad("L801","2").GetPosition()
pvos=pad("U7","6").GetPosition()
vos_a=v(109.0,14.5)
vos_b=v(104.6,14.5)
vos_c=v(104.6,12.25)
locked_track("+5V",pout,vos_a,0.20)
locked_track("+5V",vos_a,vos_b,0.20)
locked_track("+5V",vos_b,vos_c,0.20)
locked_track("+5V",vos_c,pvos,0.20)
# Tie the regulated output node into the bulk/output-capacitor cluster explicitly.
pout_bulk=pad("C803","1").GetPosition()
vbulk_a=v(109.0,18.5)
locked_track("+5V",vos_a,vbulk_a,0.30)
locked_track("+5V",vbulk_a,pout_bulk,0.30)
...
text("Rev.D-G v1.2 PCB RC9 - LOW VOLTAGE ONLY",50,82,1.3)
text("U6 RAR0017B DRAFT - RC9 routing proof only; TI official land pattern freeze pending",50,79.5,0.9)
...
# Create an RC9 project alongside the board so kicad-cli DRC uses the intended rules.
PRO_FILE=ROOT/"RevD_G_v1_2_PCB_RC9.kicad_pro"
